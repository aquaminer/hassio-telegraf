...
COLLECTD_PORT=$(bashio::config 'collectd.port')
COLLECTD_AUTH_FILE=$(bashio::config 'collectd.auth_file')
...
if bashio::var.true "${CUSTOM_CONF_ENABLED}"; then
  ...
  if bashio::config.true 'collectd.enabled'; then
    bashio::log.info "Updating config for collectd"
    {
      echo "[[inputs.socket_listener]]"
      echo "  service_address = \"udp://:COLLECTD_PORT\" "
      echo "  data_format = \"collectd\""
      echo "  collectd_auth_file = \"COLLECTD_AUTH_FILE\""
      echo "  collectd_security_level = \"COLLECTD_SECURITY_LEVEL\""
      echo "  collectd_typesdb = [\"COLLECTD_TYPES_DB\"]"
      echo "  collectd_parse_multivalue = \"split\""
    } >> $CONFIG

    sed -i "s,COLLECTD_PORT,${COLLECTD_PORT},g" $CONFIG
    sed -i "s,COLLECTD_AUTH_FILE,${COLLECTD_AUTH_FILE},g" $CONFIG
    sed -i "s,COLLECTD_SECURITY_LEVEL,${COLLECTD_SECURITY_LEVEL},g" $CONFIG
    sed -i "s,COLLECTD_TYPES_DB,${COLLECTD_TYPES_DB},g" $CONFIG
  fi
